...
void timer_non_busy_sleep(int microSecounds){
    if(!interrupts_context()) {
        int finish = timer_get_timestamp() + microSecounds;
        ...
        sema_down(&twn.sem);
        printf("\nI've been sleeping for %d Microsecounds\n", microSecounds);
        printf("now is %d\n",timer_get_timestamp());
    }
}
...
//    twn = list_entry(list_front(wait_list),struct timer_wait_node,elem);
    while (e != list_tail(wait_list)){
        ...
        unsigned time = timer_get_timestamp();
        if(twn->t->finish <= time){
            sema_up(&twn->sem);
            list_remove(&twn->elem);
        }
        e=e->next;
    }
    interrupts_set_level(old_level);
